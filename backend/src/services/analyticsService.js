    ...
    include: [
      {
        model: Player,
        as: "player",
        attributes: ["firstName", "lastName", "teamId"],
      },
    ],
  ...
  const matches = await Match.findAll({
    ...
    include: [
      ...
      { model: Team, as: "awayTeam" },
      {
        ...
        as: "playerStats",
        include: [
          { model: Player, as: "player", attributes: ["id", "teamId"] },
        ],
      },
    ],
  });
...
const headToHead = async (teamA, teamB) => {
  ...
  const summary = {
    teamA: { id: teamA, wins: 0 },
    teamB: { id: teamB, wins: 0 },
    draws: 0,
  };
  ...
  const details = matches.map((match) => {
    ...
    else if (match.homeTeamId === teamA && homeGoals > awayGoals)
      summary.teamA.wins += 1;
    else if (match.awayTeamId === teamA && awayGoals > homeGoals)
      summary.teamA.wins += 1;
...
const injuryBurden = async () => {
  ...
  const injuries = await PlayerInjury.findAll({
    where: {
      [Op.or]: [{ injuryEnd: null }, { injuryEnd: { [Op.gt]: today } }],
    },
...
const playerLoadVsActiveInjuries = async () => {
  const rows = await Player.findAll({
    attributes: [
      ...
      [
        fn("COALESCE", fn("SUM", col("matchStats.minutes_played")), 0),
        "minutes",
      ],
      [
        literal(
...
const buildPlayerStatRows = async (playerId) =>
  PlayerMatchStats.findAll({
    ...
    attributes: [
      ...
      [
        fn("SUM", col("PlayerMatchStats.shots_on_target")),
        "totalShotsOnTarget",
      ],
      [
        fn("SUM", literal("PlayerMatchStats.goals * PlayerMatchStats.goals")),
        "sumGoalsSquared",
      ],
      [
        fn(
          "SUM",
          literal("PlayerMatchStats.assists * PlayerMatchStats.assists")
        ),
        "sumAssistsSquared",
      ],
    ],
...
const buildScoreResultRows = async (playerId) =>
  Score.findAll({
    where: {
      ...(playerId ? { playerId } : {}),
      result: { [Op.not]: null },
    ...
    include: [
      { model: Player, as: "player", attributes: ["firstName", "lastName"] },
    ],
...
const consistency = async (playerId) => {
  ...
  return rows
    .map((row) => {
      ...
      const stdResult = calcStdDev(
        totalResult,
        row.get("sumResultSquared"),
        samples
      );
...
const playerVsTeam = async (playerId, teamId) => {
  ...
  const row = await PlayerMatchStats.findOne({
    attributes: [
      ["player_id", "playerId"],
      [
        fn("COALESCE", fn("SUM", col("PlayerMatchStats.goals")), 0),
        "totalGoals",
      ],
      [
        fn("COALESCE", fn("SUM", col("PlayerMatchStats.assists")), 0),
        "totalAssists",
      ],
      [
        fn("COALESCE", fn("SUM", col("PlayerMatchStats.minutes_played")), 0),
        "totalMinutes",
      ],
      [
        fn(
          "SUM",
          literal(
            "CASE WHEN PlayerMatchStats.minutes_played > 0 THEN 1 ELSE 0 END"
          )
        ),
        "appearances",
      ],
...
const topScorersByTournament = async (tournamentId, limit = 10) => {
  ...
  const rows = await PlayerMatchStats.findAll({
    ...
    order: [
      [fn("COALESCE", fn("SUM", col("PlayerMatchStats.goals")), 0), "DESC"],
    ],
...
const nationalityPerformance = async () => {
  const rows = await PlayerMatchStats.findAll({
    attributes: [
      [fn("COUNT", col("PlayerMatchStats.id")), "totalAppearances"],
      [fn("SUM", col("PlayerMatchStats.minutes_played")), "totalMinutes"],
      [fn("SUM", col("PlayerMatchStats.goals")), "totalGoals"],
      [fn("SUM", col("PlayerMatchStats.assists")), "totalAssists"],
      [fn("SUM", col("PlayerMatchStats.yellow_cards")), "totalYellowCards"],
      [fn("SUM", col("PlayerMatchStats.red_cards")), "totalRedCards"],
      [
        fn("SUM", col("PlayerMatchStats.shots_on_target")),
        "totalShotsOnTarget",
      ],
      [fn("AVG", col("PlayerMatchStats.goals")), "avgGoalsPerMatch"],
      [fn("AVG", col("PlayerMatchStats.assists")), "avgAssistsPerMatch"],
      [
        fn("AVG", col("PlayerMatchStats.yellow_cards")),
        "avgYellowCardsPerMatch",
      ],
      [fn("AVG", col("PlayerMatchStats.red_cards")), "avgRedCardsPerMatch"],
    ],
    include: [
      {
        model: Player,
    ...
    group: ["player.nationality"],
    order: [[fn("COUNT", col("PlayerMatchStats.id")), "DESC"]],
    raw: true,
  });

  return rows.map((row) => ({
    nationality: row["player.nationality"],
    totalAppearances: Number(row.totalAppearances) || 0,
    totalMinutes: Number(row.totalMinutes) || 0,
    totalGoals: Number(row.totalGoals) || 0,
    totalAssists: Number(row.totalAssists) || 0,
    totalYellowCards: Number(row.totalYellowCards) || 0,
    totalRedCards: Number(row.totalRedCards) || 0,
    totalShotsOnTarget: Number(row.totalShotsOnTarget) || 0,
    avgGoalsPerMatch: Number(row.avgGoalsPerMatch || 0).toFixed(2),
    avgAssistsPerMatch: Number(row.avgAssistsPerMatch || 0).toFixed(2),
    avgYellowCardsPerMatch: Number(row.avgYellowCardsPerMatch || 0).toFixed(2),
    avgRedCardsPerMatch: Number(row.avgRedCardsPerMatch || 0).toFixed(2),
    goalsPer90: row.totalMinutes
      ? Number((row.totalGoals / row.totalMinutes) * 90).toFixed(2)
      : "0.00",
    assistsPer90: row.totalMinutes
      ? Number((row.totalAssists / row.totalMinutes) * 90).toFixed(2)
      : "0.00",
  }));
};
...
const presenceImpact = async (playerId, opponentTeamId) => {
  ...
  const aggregates = matches.reduce((acc, match) => {
    const played = match.playerStats?.some(
      (stat) => stat.playerId === playerId && Number(stat.minutesPlayed) > 0
    );
    const teamScore =
      match.homeTeamId === player.teamId ? match.homeScore : match.awayScore;
  ...
  return Object.values(aggregates)
    .map((bucket) => ({
      ...
      avg_result: bucket.samples
        ? Number((bucket.total / bucket.samples).toFixed(2))
        : 0,
