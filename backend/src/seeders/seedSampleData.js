...
const buildMatchKey = ({ homeTeamId, awayTeamId, matchDate }) =>
...
const seedSampleData = async () => {
  ...
  for (const payload of playerPayloads) {
    ...
    // Ensure birthdate is populated even if the record existed previously
    if (!player.birthdate || player.birthdate === "") {
      await player.update({ birthdate: payload.birthdate });
    }
    players.push(player);
  }
  const playerLookup = new Map(
    players.map((p) => [`${p.firstName} ${p.lastName}`, p])
  );
  ...
  const matchDetails = [
    ...
    {
      key: buildMatchKey(matchPayloads[3]),
      stats: [
        {
          player: "Amir Rahman",
          minutesPlayed: 90,
